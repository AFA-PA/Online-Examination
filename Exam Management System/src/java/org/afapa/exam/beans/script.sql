drop database ExamManDb;
create database ExamManDb;
use ExamManDb;
...
CREATE TABLE EXAM (ID BIGINT NOT NULL, MINUTESALLOWED INTEGER, NAME VARCHAR(255), STARTTIME DATETIME, REGISTRATION_id BIGINT, PRIMARY KEY (ID));
...
CREATE TABLE CourseRegistration (id BIGINT AUTO_INCREMENT NOT NULL, semester VARCHAR(255), year INTEGER, course_id BIGINT, PRIMARY KEY (id));
...
CREATE TABLE UserRegistration (registeredUsers_ID BIGINT NOT NULL, reg_id BIGINT NOT NULL, PRIMARY KEY (registeredUsers_ID, reg_id));
...
ALTER TABLE EXAM ADD CONSTRAINT FK_EXAM_REGISTRATION_id FOREIGN KEY (REGISTRATION_id) REFERENCES CourseRegistration (id);
...
ALTER TABLE CourseRegistration ADD CONSTRAINT FK_CourseRegistration_course_id FOREIGN KEY (course_id) REFERENCES ORGANIZATIONALUNIT (ID);
...
ALTER TABLE UserRegistration ADD CONSTRAINT FK_UserRegistration_registeredUsers_ID FOREIGN KEY (registeredUsers_ID) REFERENCES USER (ID);
ALTER TABLE UserRegistration ADD CONSTRAINT FK_UserRegistration_reg_id FOREIGN KEY (reg_id) REFERENCES CourseRegistration (id);
