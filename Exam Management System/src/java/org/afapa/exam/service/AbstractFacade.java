...
public abstract class AbstractFacade<T> {
    ...
    protected abstract UserIdentityStore getIdentityStore();

    public <E> CredentialValidationResult validateRequest(SignedinRequest<E> request) {
        User user = getEntityManager().createNamedQuery("getUserById", User.class)
                .setParameter("id", request.getUserId()).getSingleResult();
        return getIdentityStore().validate(new UserCredential(user,
                new Token(user, request.getUserToken())));
    }
